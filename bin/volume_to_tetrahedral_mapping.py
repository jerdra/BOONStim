#!/usr/bin/env python
'''
Given a ribbon constrained volume image with weights per voxel.
Project the data into tetrahedral space using the
tetrahedral projection algorithm.

Usage:
    volume_to_tetrehedral_mapping <vol> <FEM> <out>

Arguments:
    <vol>                               Ribbon constrained volume file
                                        (.nii.gz)
    <FEM>                               GMSH .msh file
    <out>                               Output file
                                        (weight per tetrahedron array)
...
def main():
    ...
    # Determine whether the file is parcellation- or weight-based
